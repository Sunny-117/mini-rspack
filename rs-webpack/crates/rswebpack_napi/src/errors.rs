...
use rswebpack_error::NodeError;

pub trait NapiErrorExt {
    fn into_rswebpack_error(self) -> rswebpack_error::Error;
    fn into_rswebpack_error_with_detail(self, env: &Env) -> rswebpack_error::Error;
}

pub trait NapiResultExt<T> {
    fn into_rspack_result(self) -> rswebpack_error::Result<T>;
    fn into_rspack_result_with_detail(self, env: &Env) -> rswebpack_error::Result<T>;
}
...
impl<T: 'static> NapiResultExt<T> for Result<T> {
    fn into_rspack_result(self) -> rswebpack_error::Result<T> {
        self.map_err(|e| e.into_rswebpack_error())
    }
    fn into_rspack_result_with_detail(self, env: &Env) -> rswebpack_error::Result<T> {
        self.map_err(|e| e.into_rswebpack_error_with_detail(env))
    }
}
