...
use rswebpack_error::{miette::IntoDiagnostic, Error, Result};
...
impl<T: 'static, R> ThreadsafeFunction<T, R> {
    async fn resolve_error(&self, err: napi::Error) -> Error {
        let (tx, rx) = tokio::sync::oneshot::channel::<rswebpack_error::Error>();
        self.resolver.call(Box::new(move |env| {
            let err = err.into_rswebpack_error_with_detail(&env);
