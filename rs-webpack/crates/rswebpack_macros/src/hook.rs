    ...
    pub fn expand(self) -> Result<TokenStream> {
        ...
        Ok(quote! {
          ...
          impl rswebpack_hook::Hook for #hook_name {
            ...
            fn used_stages(&self) -> rswebpack_hook::__macro_helper::FxHashSet<i32> {
              rswebpack_hook::__macro_helper::FxHashSet::from_iter(self.taps.iter().map(|h| h.stage()))
            }

            fn intercept(&mut self, interceptor: impl rswebpack_hook::Interceptor<Self> + Send + Sync + 'static) {
    ...
    pub fn return_type(&self) -> TokenStream {
        match self {
            Self::AsyncSeriesBail { ret } | Self::SyncSeriesBail { ret } => {
                if let Some(ret) = ret {
                    quote! { rswebpack_hook::__macro_helper::Result<std::option::Option<#ret>> }
                } else {
                    quote! { rswebpack_hook::__macro_helper::Result<std::option::Option<()>> }
                }
            }
            Self::AsyncSeriesWaterfall { ret } => {
                quote! { rswebpack_hook::__macro_helper::Result<#ret> }
            }
            _ => quote! { rswebpack_hook::__macro_helper::Result<()> },
        }
    }
